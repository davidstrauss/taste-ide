...
- **Decide machine sizing as a commitment.** Host RSS ratchets to the
  configured memory and never returns it; there is no balloon.
...
| Artifact | Removal |
| --- | --- |
| Machine `taste-spike` (qcow2 ~2.0 GB) | `podman machine stop taste-spike && podman machine rm -f taste-spike` |
...
| Work dir: repo copy at HEAD, build logs, `baseline.tar`, `systemd.tar`, target dirs | `rm -rf ~/.local/share/taste-spike` |
