...
- **A launch stops the VMs nobody owns.** Before this workspace asks the
  host for room, every running Taste VM of another workspace whose window
  is gone — the domain wears another prefix, and its XML names no folder
  or nobody holds that folder's supervision lock, probed without claiming
  it — gets its ACPI shutdown, said once in a toast naming the projects;
  when the host was at capacity, the bring-up waits for them to be down
  first (David, 2026-09-22: "simply stop any local Taste IDE VMs that
