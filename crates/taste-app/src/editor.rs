...
/// display. Writes here go straight to disk, so there is never unsaved
/// state hiding behind an invisible tab.
...
impl EditorPage {
    fn file_format(&self) -> FileFormat {
        FileFormat {
            crlf: self.crlf.get(),
            bom: self.bom.get(),
            trim_trailing_ws: self.trim_trailing_ws.get(),
            final_newline: self.final_newline.get(),
            eol_override: self.eol_override.get(),
        }
    }

    fn set_file_format(&self, format: &FileFormat) {
        self.crlf.set(format.crlf);
        self.bom.set(format.bom);
        self.trim_trailing_ws.set(format.trim_trailing_ws);
        self.final_newline.set(format.final_newline);
        self.eol_override.set(format.eol_override);
    }
}
...
impl Editor {
    pub fn new(workspace: taste_core::Workspace) -> Rc<Self> {
        ...
        // The way to a tab that scrolled off, and GNOME Builder's: a menu
        // of the strip's pages, at the strip's end (`crate::pages_menu`).
        // At the consolidated rung this one strip carries the files, the
        // chat's faces and the console's tabs, and at 900px about four of
        // them are on screen at a time. Before the console's + when that
        // arrives, so "add one more tab" keeps the far right end.
...
/// Agent-facing buffer access — the ACP `fs/read_text_file` and
/// `fs/write_text_file` path.
///
/// A file the user has open is read and written as the buffer they can see,
/// so the agent gets their unsaved edits and their tab stays put. A file
/// they have NOT opened never becomes a widget: it is text plus a
/// [`FileFormat`], written through the same `taste_core::textfile` code the
...
impl Editor {
    ...
    /// open (unsaved edits included), else the headless copy, else `None`
    /// for "read the file yourself".
    pub fn buffer_text(&self, path: &Path) -> Option<String> {
        if let Some(page) = self.pages.borrow().get(path) {
            let (start, end) = page.buffer.bounds();
            return Some(page.buffer.text(&start, &end, true).to_string());
        }
        self.headless.borrow().get(path).map(|b| b.text.clone())
    }

    /// Whether `path` has edits the user has not saved. Headless buffers
    /// are written through on every change, so only a real tab can be
    /// unsaved.
    ...
    /// their unsaved work" half of settling an open file.
    ...
    pub fn buffer_revert(self: &Rc<Self>, path: &Path) -> Result<(), String> {
        ...
        if let Some(page) = self.pages.borrow().get(path).cloned() {
            ...
            page.saved_hash
                .set(Some(content_hash(&textfile::render(&disk, &format))));
            return Ok(());
        }
        self.headless.borrow_mut().remove(path);
        Ok(())
    ...
    /// Release a buffer the agent opened. A file the user has since opened
    /// is theirs — closing it under them is not ours to do.
    pub fn buffer_close(&self, path: &Path) {
        self.headless.borrow_mut().remove(path);
    }

    /// Load-or-create the headless copy, apply `edit`, and write it out.
    /// No GTK on this path at all.
    fn write_headless(
        &self,
        path: &Path,
