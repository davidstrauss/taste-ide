...
/// How many environments beyond the primary an ORCHESTRATOR may have
/// **running** at once.
///
/// Running, not existing — and the difference is the whole of the number's
/// meaning (i-0013). A running environment is a container, an agent
...
/// has not planned, it has thrashed. All three of those are released the
/// moment the environment stops, which is what flagging one for review
/// does: what survives a stop is the clone on disk, the cheapest of the
...
/// Environment", and the row's Start action are a person's decision about
/// their own machine and stay unbounded. The tool refuses by naming the cap
