...
/// A short, stable name for one checkout.
///
/// `taste_core::environment` owns every derived name in the IDE, and this
/// used to be a second one — a `DefaultHasher` over the path, on the
/// reasoning that a directory reconciled on every spawn need only separate
/// concurrent agents and not survive a release. True of the stub directory,
/// and it made this the one key in the codebase that did not follow
/// symlinks and did not share a generation with the rest. Two names for one
/// concept is how the first one drifts, so there is one now.
use taste_core::environment::workspace_key;
...
///
/// **Per workspace**, because the watcher on the other end is a window and
/// windows are per folder. One shared directory made three things go wrong
/// at once with two windows open: each window purged the whole directory at
/// startup, deleting a sign-in URL another window's agent had just dropped;
/// each window watched all of it, so whichever window's dialog appeared
/// first consumed the URL, and a person could be asked to sign in for a
/// project they were not looking at while the agent that needed it hung;
/// and one project's agent could read another's pending sign-in URLs, which
/// is a boundary the IDE has no business blurring.
pub fn url_bridge_dir(workspace_root: &Path) -> PathBuf {
    cache_dir()
        .join("agent-urls")
        .join(taste_core::environment::workspace_key(workspace_root))
}
...
///
/// The script is written beside the drop directory rather than inside it:
/// the drop directory is the one place in the bridge an agent can write, and
/// the helper it is about to execute should not be somewhere it can rewrite
/// on the way. Its name carries the workspace key for the same reason the
/// directory does — two windows must not overwrite each other's helper, each
/// of which names a different directory.
pub fn ensure_url_bridge(workspace_root: &Path) -> Result<(PathBuf, PathBuf)> {
    let dir = url_bridge_dir(workspace_root);
    std::fs::create_dir_all(&dir).context("creating agent-urls dir")?;
    let script_path = dir.parent().unwrap_or(Path::new("/tmp")).join(format!(
        "taste-open-url-{}",
...
mod tests {
    ...
    /// A sign-in URL belongs to the window whose agent asked for it.
    ///
    /// One shared drop directory made three things go wrong the moment a
    /// second window opened: each window purged the whole directory at
    /// startup and deleted the other's live URLs, each watched all of it so
    /// the wrong window could claim a URL and leave the right agent hanging,
    /// and one project's agent could read another's pending sign-ins. All
    ...
    fn url_helper_script_writes_into_drop_dir() {
        ...
        let (script, drop_dir) = ensure_url_bridge(Path::new("/work/project")).unwrap();
        // A second window's bridge, written while the first exists.
        let (other_script, other_drop) = ensure_url_bridge(Path::new("/work/other")).unwrap();
        std::env::remove_var("XDG_CACHE_HOME");
        assert!(script.is_file());
        let content = std::fs::read_to_string(&script).unwrap();
        assert!(content.contains(&drop_dir.display().to_string()));

        // Two windows, two helpers naming two directories. One shared
        // helper would have had the second window's agents dropping URLs
        // into the first window's directory, which is where the whole
        // cross-window mix-up started.
        assert_ne!(script, other_script);
        assert_ne!(drop_dir, other_drop);
        assert!(other_drop.is_dir());
        assert!(std::fs::read_to_string(&other_script)
            .unwrap()
            .contains(&other_drop.display().to_string()));
        // The helper is not inside the one directory the agent may write.
        assert_ne!(script.parent(), Some(drop_dir.as_path()));
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
